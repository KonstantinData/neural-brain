...
| Memory authority grants | Structured, issuer-bound, expiry-bound catalog and immutable snapshot primitive implemented; no gate integration or capability release | `migrations/0009_memory_authority_grants.sql`; `security/authority.py`; unit and migration contract tests | Prove catalog administration, revocation propagation, and Gate-owned use only after an accepted operation-specific Control Plane contract; grants alone never authorize a transition. |
