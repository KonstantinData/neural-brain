...
| Memory authority grants | Structured, issuer-bound, expiry-bound catalog; deterministic resolver; and canonical immutable snapshot evidence implemented; no gate integration or capability release | `migrations/0009_memory_authority_grants.sql`; `migrations/0010_memory_authority_snapshot_digests.sql`; `security/authority.py`; unit and migration contract tests | Prove catalog administration, revocation propagation, and Gate-owned use only after an accepted operation-specific Control Plane contract; grants alone never authorize a transition. |
